...
function setProxy(
  ...
  // that drops the proxy or crosses an HTTPSÃ¢â€ â€HTTP boundary doesn't reuse a
  ...
  if (proxy) {
    ...
    // direct reads are shielded), but plain object proxies Ã¢â‚¬â€ and the `auth` field
    // (which URL does not expose) Ã¢â‚¬â€ must be guarded so a polluted Object.prototype
    ...
    if (targetIsHttps) {
      ...
      // the origin so the proxy cannot inspect the URL, headers, or body Ã¢â‚¬â€ the
      ...
      // the absolute URL and the proxy sees everything Ã¢â‚¬â€ acceptable for plain
...
const normalizeLookup = (lookup) => {
  ...
  const callbackLookup = callbackify(lookup, (value) => (utils.isArray(value) ? value : [value]));
