...
function flatten(array) {
  ...
  // which is O(nÃ‚Â²) once a query resolves to many nodes.
  var result = []
  var stack = [array]
  while (stack.length) {
    var item = stack.pop()
...
// a suffix one character at a time and re-tested anchored `\s+Ã¢â‚¬Â¦` regexps at
// every length, which backtracks across whitespace runs and is O(nÃ‚Â²) Ã¢â‚¬â€ a
...
function parseBlock(all, block, qs) {
  ...
  while (i < len) {
    ...
    if (ch === ',') {
      ...
      // clause Ã¢â‚¬â€ the original never emits one there.
